<template>
  <div class="Publications page bg-white">
    ...
    <div v-if="data" class="content grid xl:grid-cols-[300px_1fr] gap-16">
      <div class="filters flex flex-wrap gap-1.5 content-start">
        <button aria-active="true" data-tag="all" class="border border-bleu px-3 pt-1 pb-0.5 rounded-full aria-[active=false]:opacity-60" @click="filterPost">
          Tout
        </button>
        <button v-for="tag in getTags(data)" aria-active="false" :data-tag="clearTag(tag)" class="border border-bleu px-3 pt-1 pb-0.5 rounded-full aria-[active=false]:opacity-60" @click="filterPost">
          {{tag}}
        </button>
      </div>
